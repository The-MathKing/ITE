...
whether the current sample repeats the one $k$ steps earlier, the primitive
behind cycle detection by random-walk graph learners. For Gaussian inputs,
exact detection requires $S\ge k+1$ and a singular state matrix, so it is
unavailable to invertible dynamics such as S4D and Mamba. The lag-$k$ delay
has $k$ unit Hankel singular values. Hence for $S<k$ no linear probe beats
the zero probe in Hankel norm, any probe explains at most $S/k$ of the
delayed sample's variance, and every readout has detection divergence at
most $\tfrac12\ln\frac{k}{k-S}$. Optimal fits track the $S/k$ law, and a
trained shift register switches on at $S=k+1$. Trained diagonal SSMs respect
the bounds but need \ratioLTIc$\times$ the minimal state; freezing their
poles at the optimal fit removes most of this overhead for $k\le12$.
\end{abstract}
...
from LTI (paired over $S$ and seeds). By the token-level controls, the
likely bottleneck is learning precise poles for several class-specific lags
from a pooled graph label, but we do not isolate it here. Class $r=2$ ($W^*=3$) is learned at $S=2$ only by
...
input-dependent step does not change it. \CSLConclusion{} Lower bounds for
