...
Optimal fits follow the $S/k$ law, and a trained shift register switches on
exactly at $S=k+1$. Trained diagonal SSMs stay below the Gaussian ceiling but
need \ratioLTIc$\times$ the minimal state to reach an AUROC of $0.95$.
Freezing their poles at the optimal fit raises the AUROC at $S=k$ from
\ltiAtK{} to \frozenAtK, so most of this overhead lies in learning the
dynamics. On circular skip-link graphs, non-backtracking return horizons
predict when exact-count features separate classes.
\end{abstract}
...
The surrogate is a fair model on these graphs. A Bayes-optimal table of the
exact revisit indicators between $x_t$ and lags $1,\dots,S-1$ predicts lag-$k$
revisits at AUROC \leakLo--\leakHi, so this short-lag walk structure carries no
information about lag-$k$ revisits. The input-dependent step changes the
AUROC by \selDiff$\pm$\selCI{} (mean over cells; Wilcoxon $p=\selP$). The
change is consistent but negligible. Its learned $\Delta_t$ varies by only
\dtCV\% across tokens, which is expected because random features give the gate
...
Proposition~\ref{prop:exact}(c); the small excess over $0.5$ at $S=k$ comes
from walk structure among the stored lags.
...
ill-conditioned (Remark~1).
\item \emph{Comparison features.} Removing them changes the $k=8$ AUROC by at
most \nocmpMaxDiff{} at any $S$.
\end{itemize}
The information limit is therefore nearly attainable at $S\approx k$. Most of
the overhead of trained SSMs comes from learning the dynamics, not from what
an order-$S$ state can represent or from the readout.
