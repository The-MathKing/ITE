...
requires $S\ge k+1$ and a singular state matrix, so LTI invertible dynamics
such as S4D (or Mamba with a fixed step) cannot achieve it. Since the lag-$k$ delay has $k$ unit
...
\ratioLTIc$\times$ the minimal state to reach AUROC $0.95$; frozen optimal
poles remove most of this overhead for $k\le12$. On circular skip-link
graphs the remaining gap lies downstream of the dynamics.
\end{abstract}
...
$H_1$: $x_t=x_{t-k}$ against $H_0$: $x_t$ fresh, from $(s_t,x_t)$ of a
stable LTI model. (a) If $S\le k$, or (b) if $A$ is invertible (any $S$), every readout
...
either equivalent (error $>0$) or mutually singular. The support under
$H_1$ lies in that under $H_0$, where $x_t$ is free given $s_{t-1}$. So
singularity needs $a^\top s_t+b^\top x_t=0$ a.s.\ under $H_1$ but not under
$H_0$. Since
...
At $S=k$ the error in (a) can nonetheless be made arbitrarily small
(Theorem~\ref{thm:hankel}), so the threshold $S\ge k+1$ concerns exactness,
...
Every S4D discretisation $A=\exp(\Delta\Lambda)$, and Mamba's with a fixed
step, is invertible, so exact detection is never available to these LTI
models. The relevant question is how
...
carries divergence at most about $S/(2\ell)$.
...
i.e.\ not at all. Within two lags of $k$ they reach up to \pairKmTwoHi{} ($S=k-2$),
...
$S=\cslShiftBestS$. On CSL the gap therefore lies downstream of the dynamics,
in the readout and mean pooling. Graph-level labels alone do not explain it,
since the LTI model with a token-level revisit loss does no better. The shift
register still tests Proposition~\ref{prop:exact}(c) at the graph level: it
first reaches 80\% on classes $r=2$, $3$ and $13$ at $S=4$, $6$ and $6$
(Table~\ref{tab:csl}). This is the first tested $S\ge W^*(r)+1$, the
smallest register that holds lag $W^*(r)$. Class $r=2$ ($W^*=3$) is learned at $S=2$ only by
...
so there the bottleneck lies downstream of the dynamics, in the readout and
pooling. Lower bounds for
input-dependent dynamics and better-conditioned delay parameterisations are
natural next steps.

\section*{Acknowledgment}
The authors used Claude (Anthropic), a generative AI system, extensively in
this work. It wrote the experiment and analysis code, ran the experiments and
produced all figures and tables, drafted and revised the text of every
section, including the statements and proofs in Section~III, and carried out
simulated pre-submission reviews whose findings led to corrections. All code,
data and generated numbers are released with the article. The authors take
